...
module.exports = function(Desiredlanguages) {

    var getPromise=function(sqlString, param){
        var promise = new Promise((resolve, reject) => { 
            var ds = Desiredlanguages.dataSource;
            var sql = sqlString;
    ...
    Desiredlanguages.beforeRemote('create', function(ctx, unused, next) {
    ...
    Desiredlanguages.beforeRemote('deleteById', function(ctx, unused, next) {
        if(ctx.req.session.cas_user) {
            var param = ctx.req.session.cas_user;
            var sql ="select id from partner where cas_user = ?";
            var getUserId = getPromise(sql, param, ctx);
            getUserId.then((result)=>{
                ...
                var sql ="select user_id from desired_languages where id = ?";
