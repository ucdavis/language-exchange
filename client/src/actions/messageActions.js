...
export function fetchSentMessages(id){
    return function(dispatch){
        axios.get(`/api/contacts?filter={"include":{"relation":"recipient"},"where":{"sender_id":${id}}}`)
...
export function fetchReceivedMessages(id){
    return function(dispatch){
        axios.get(`/api/contacts?filter={"include":{"relation":"sender"},"where":{"recipient_id":${id}}}`)
